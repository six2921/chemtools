...
def assay(target_chembl_id, pchembl_value_isnull=False):
    import pandas as pd
    from chembl_webresource_client.new_client import new_client

    # CHEMBL API로 assay를 json으로 받아오기
    activity = new_client.activity
    res = activity.filter(target_chembl_id=target_chembl_id, pchembl_value_isnull=pchembl_value_isnull)
    print(f"# Assay values in {target_chembl_id}: ", len(res))

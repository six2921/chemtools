...
ignore_list = open('check_pdb_ignore_list.txt', 'r').read().split('\n')
...
def pdb2ligands(pdb):
    ...
    if 'non_polymer_entity_ids' not in res.json()['rcsb_entry_container_identifiers']:
        pass
    else:
        comp_list = dict()
...
def uniprot2pdbs(uniprot_id):
    ...
    df = pd.read_json(json.dumps(res.json()[f"{uniprot_id}"]))
...
def uniprot2pdbligands(uniprot_id, count_cutoff=100):
    ...
    for idx in df.index:
        ...
        if pdb2ligands(pdb) is not None:
            ...
            """
            # pdb2ligands 함수가 리간드를 찾으면 +1, 5개 이상 찾으면 루프를 멈춤.
            if len(lig_dict.keys()) > 0 :    
                lig_count += 1
            if lig_count >= count_cutoff :
                break
            """
